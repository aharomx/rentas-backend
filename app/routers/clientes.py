...
def read_direcciones(cliente_id: int, db: Session = Depends(get_db)):
...
def create_direccion(cliente_id: int, direccion: DireccionCreate, db: Session = Depends(get_db)):
...
def read_contactos(cliente_id: int, db: Session = Depends(get_db)):
...
def read_contactos_by_role(cliente_id: int, rol: str, db: Session = Depends(get_db)):
...
def create_contacto(cliente_id: int, contacto: ContactoCreate, db: Session = Depends(get_db)):
    cliente = crud_cliente.get_cliente(db, cliente_id)
    if not cliente:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")
